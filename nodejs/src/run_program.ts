...
const runCompiledLanguageProgram = (
  installedConvertor: string,
  code: string,
  extension: string
): string => {
...
const runInterpretedLanguageProgram = (
  installedConvertor: string,
  code: string,
  extension: string
): string => {
  fs.writeFileSync(`tmp.${extension}`, code);
  const output: string = child
    .execSync(`${installedConvertor} tmp.${extension}`)
    .toString();
  child.execSync(`rm tmp.${extension}`);
  return output;
};
...
const runProgram = (
  ...
  switch (language) {
    ...
    case "go":
      return runCompiledLanguageProgram(
        `${installedConvertor} build`,
        code,
        "go"
      );
    case "javascript":
